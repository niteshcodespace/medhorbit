...
type EvaluatorParseResult =
  | { ok: true; results: EvaluatorQuestionResult[] }
...
function buildEvaluationPrompt(input: WorksheetValidationInput): string {
  ...
  return [
    ...
    "Review every question below as a quality heuristic; you may be wrong, so judge conservatively.",
    ...
    questionLines,
    "",
    `For each of the ${input.questions.length} questions, judge:`,
    "- answerValid: is the given answer correct or sensible for the question?",
    "- difficultyValid: does the question match the requested difficulty?",
    "- topicValid: is the question actually about the requested topic?",
    "- ageAppropriate: is the question appropriate for this class/age group?",
    "",
    'Respond with JSON only, in the form {"questions":[{"index":0,"answerValid":true,"difficultyValid":true,"topicValid":true,"ageAppropriate":true,"reason":""}]}.',
    "Include exactly one result per question, using the same index shown above. Keep \"reason\" short and only fill it in when something is invalid.",
  ].join("\n");
}
...
function parseEvaluatorResponse(text: string, expectedCount: number): EvaluatorParseResult {
  ...
  for (const item of questions) {
    ...
    const { index, answerValid, difficultyValid, topicValid, ageAppropriate, reason } =
    ...
    results.push({ index, answerValid, difficultyValid, topicValid, ageAppropriate, reason });
  }
